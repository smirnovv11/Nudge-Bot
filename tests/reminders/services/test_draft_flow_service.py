...
from nudge_bot.reminders.services.schemas import DraftActionOutcomeEnum
...
async def test_confirm_draft_creates_reminder_once_and_marks_confirmed() -> None:
    ...
    assert first.outcome == DraftActionOutcomeEnum.CONFIRMED
    ...
    assert second.outcome == DraftActionOutcomeEnum.ALREADY_CONFIRMED
...
async def test_confirm_voice_draft_preserves_source_metadata() -> None:
    ...
    draft = Draft(
        ...
        parse_confidence=0.6,
        payload={
            SOURCE_TYPE_PAYLOAD_KEY: ReminderSourceTypeEnum.VOICE.value,
    ...
    assert result.outcome == DraftActionOutcomeEnum.CONFIRMED
    assert result.reminder is reminders.added[0]
    assert result.reminder.source_type == ReminderSourceTypeEnum.VOICE
...
async def test_confirm_draft_marks_expired_draft_without_creating_reminder() -> None:
    ...
    assert result.outcome == DraftActionOutcomeEnum.EXPIRED
...
async def test_cancel_draft_marks_cancelled_once() -> None:
    ...
    assert first.outcome == DraftActionOutcomeEnum.CANCELLED
    assert first.changed is True
    assert draft.status == DraftStatusEnum.CANCELLED
    assert second.outcome == DraftActionOutcomeEnum.ALREADY_CANCELLED
    assert second.changed is False
    assert reminders.added == []
    assert drafts.locked_lookup_called is True
